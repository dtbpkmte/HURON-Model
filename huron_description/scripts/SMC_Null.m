...
% L1=2;    k1=-150;   p1=0.001;    c1=0.0001;    Q1=0.0001;   a1=0.0001;   z1=0.0001; % new
L1=2;    k1=-0.002;    p1=0.001;    c1=0.00001;    Q1=0.00001;      a1=0.001;         z1=0.9; %old

s_linear_motion = error_dot_in_x + L1*error_in_x; 
f1= c1 * ( 1 - exp( k1*(abs(s_linear_motion))^p1 ) );
s_linear_motion_dot= -Q1 * ( (abs(s_linear_motion))^f1 ) *sign(s_linear_motion) - z1 * (abs(s_linear_motion))^a1  * s_linear_motion ;
...
% K_angular_momentum = 5 ;
% Desired_Angular_Momentum =   K_angular_momentum * ( (calcCOP(r1_ft, l1_ft)) - (X_COM) ) ; 

% second approach : desired angular momentum
Desired_Angular_Momentum =   (m1+m2+m3) * (s_linear_motion_dot - L1 * error_dot_in_x) * ( (calcCOP(r1_ft, l1_ft)) - X_COM ) ; 
...
Desired_Theta_ddot =  (pinv( [ J_X_COM ; A_theta ])) *  ( [  ( s_linear_motion_dot - L1 * error_dot_in_x - J_X_COM_dot * theta_dot )  ;  ( Desired_Angular_Momentum - A_theta_dot * theta_dot ) ] ) ;
...
% K_q_d = diag([0.75 0.75 0.75]);
ksi_2_simp = -K_q_d*theta_dot + ksi_1 + K_q_i*sum_theta_error;
...
T_posture = M*Phi_N;
T_posture_plot(:, end+1) = T_posture;
...
T = Torque_SMC_Linear_plus_angular_compensation + T_posture ;
...
legend("hip pitch")
subplot(4,3,7);
plot(timeStamps, T_posture_plot(1, :),"Color",'r');
legend("T_posture ankle")
subplot(4,3,8);
plot(timeStamps, T_posture_plot(2, :),"Color",'c');
legend("T_posture knee")
subplot(4,3,9);
plot(timeStamps, T_posture_plot(3, :),"Color",'b');
legend("T_posture hip")
subplot(4,3,10);
